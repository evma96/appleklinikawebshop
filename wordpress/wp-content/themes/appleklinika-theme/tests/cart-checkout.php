...
$test->assert(is_string($script) && str_contains($script, "data-ak-checkout-review-step") && str_contains($script, 'Kapcsolattartás') && str_contains($script, 'Kézbesítés') && str_contains($script, 'Számlázás és fizetés') && str_contains($script, 'setActiveStep(Number(button.getAttribute'), 'Step 4 groups review data by customer intent and keeps edit navigation inside the existing stepper state.');
...
$test->assert(is_string($script) && str_contains($script, 'function currentShippingReview()') && str_contains($script, 'function currentPaymentReview()') && str_contains($script, 'function shippingReviewLines(shippingMethod)') && ! str_contains($script, "currentPaymentReview() {\n      return 'Banki átutalás'"), 'Step 4 reads current native shipping and payment selections and suppresses only redundant free-shipping wording.');
$test->assert(is_string($script) && str_contains($script, 'note && note.value.trim()') && str_contains($script, 'data-ak-checkout-review-step') && ! str_contains($script, "textContent = 'Megrendelés'"), 'Step 4 shows a note only when present, retains native edit navigation, and does not add a second order action.');
...
$test->assert(is_string($css) && str_contains($css, '.ak-checkout-final-review__section') && str_contains($css, '.ak-checkout-final-review__subsection') && str_contains($css, '.ak-checkout-summary__details') && str_contains($css, 'ak-checkout-step-4 .ak-checkout-summary__details'), 'Step 4 is one vertically structured review sheet and hides only duplicated sidebar context on the final step.');
