...
$test->assert(is_string($script) && str_contains($script, 'function syncCheckoutFinalReview()') && str_contains($script, 'Ellenőrizd a rendelésed') && str_contains($script, 'Mielőtt véglegesíted'), 'Step 4 adds one clear final review before the native legal acceptance and order action.');
$test->assert(is_string($script) && str_contains($script, "data-ak-checkout-review-step") && str_contains($script, 'setActiveStep(Number(button.getAttribute'), 'Step 4 review edit actions return through the existing stepper state without a page reload.');
...
$test->assert(is_string($script) && str_contains($script, 'function currentShippingReview()') && str_contains($script, 'function currentPaymentReview()') && ! str_contains($script, "currentPaymentReview() {\n      return 'Banki átutalás'"), 'Step 4 reads the current native shipping and payment selections without gateway-specific review copy.');
...
$test->assert(is_string($css) && str_contains($css, '.ak-checkout-final-review__groups') && str_contains($css, 'grid-template-columns: repeat(2, minmax(0, 1fr));') && str_contains($css, '.ak-checkout-summary-slot--final-review'), 'Step 4 review has a compact desktop grid and a scoped mobile presentation for the existing live summary.');
