...
$test->assert(is_string($script) && str_contains($script, 'clearInactiveBillingValidationErrors') && str_contains($script, 'clearValidationError') && str_contains($script, 'appleklinika:checkout-company-mode-changed') && str_contains($script, 'appleklinika:checkout-address-mode-changed'), 'Checkout clears stale inactive personal, company and saved-address validation errors through the supported Blocks validation store.');
$test->assert(is_string($script) && str_contains($script, 'billingPersonalNameValidation') && str_contains($script, 'companyBillingValidation') && str_contains($script, 'shippingPersonalNameValidation') && str_contains($script, 'checkoutAddressMode'), 'Checkout scopes name validation to the currently active personal, company, saved-billing or saved-shipping mode.');
