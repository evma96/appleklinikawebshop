...
$test->assert(is_string($script) && str_contains($script, 'var effectiveBilling = billing && billing.address_1 ? billing : shipping;') && str_contains($script, "['Számlázási cím', addressSummary(effectiveBilling)]"), 'Checkout Step 3 presents the effective shared physical address as billing when WooCommerce has no separate billing address.');
