...
$test->assert(is_string($script) && str_contains($script, 'clearInactiveBillingValidationErrors') && str_contains($script, 'clearValidationError') && str_contains($script, 'appleklinika:checkout-company-mode-changed') && str_contains($script, 'appleklinika:checkout-address-mode-changed') && str_contains($script, 'refreshCurrentValidationNotice'), 'Checkout clears stale inactive and corrected validation entries through the supported Blocks validation store and refreshes the existing step notice.');
$test->assert(is_string($script) && str_contains($script, 'billingFirstNameValidation') && str_contains($script, 'billingLastNameValidation') && str_contains($script, 'companyNameValidation') && str_contains($script, 'taxNumberValidation') && str_contains($script, 'shippingFirstNameValidation') && str_contains($script, 'shippingLastNameValidation') && str_contains($script, 'checkoutTaxNumberIsValid'), 'Checkout clears only validation entries whose current personal, company, tax or shipping field is valid, while preserving invalid active fields.');
$test->assert(is_string($script) && str_contains($script, 'keepCompanyFieldsInOwnedStep') && str_contains($script, 'sourceStep.classList.remove(\'ak-checkout-company-source-hidden\')') && ! str_contains($script, 'function moveCompanyFieldsIntoBillingSection') && ! str_contains($script, 'slot.insertBefore(purchaseField.wrapper'), 'Company controls stay in their Woo Blocks owned checkout step, so a cart or step remount cannot detach their React-managed DOM nodes or leave an inactive overlay above them.');
...
$test->assert(is_string($script) && str_contains($script, 'focusValidationError') && str_contains($script, "document.getElementById(matching[0].key)") && str_contains($script, "field.closest('[aria-hidden=\"true\"], .is-hidden, .is-company-hidden')"), 'Stepper returns focus only to the first visible active invalid Block field.');
$test->assert(is_string($script) && str_contains($script, 'function firstPreferredValidationField(step, entries)') && str_contains($script, "['shipping-first_name', shippingFirstNameValidation]") && str_contains($script, "['billing-first_name', billingFirstNameValidation]") && str_contains($script, "['order-appleklinika-company_name', companyNameValidation]"), 'Step 2 focuses the visible active company, personal or one-off shipping identity field instead of an unrelated later address error.');
$test->assert(is_string($script) && str_contains($script, 'function activeIdentityValidationErrors(step)') && str_contains($script, 'function completeCurrentStepValidation(onValid, errors)') && str_contains($script, 'if (immediateErrors.length)') && str_contains($script, 'validationInProgress'), 'Step 2 atomically denies an invalid active company, personal or shipping identity state before asynchronous Blocks validation may advance the remounted step.');
$test->assert(is_string($script) && str_contains($script, 'function uniqueValidationErrors(entries)') && str_contains($script, 'window.requestAnimationFrame(function ()') && ! str_contains($script, 'onValid();\n      }, 0);'), 'Step navigation waits for a rendered Woo Blocks validation result and deduplicates current-step errors instead of advancing from a zero-delay read.');
$test->assert(is_string($script) && str_contains($script, 'function bindCompanyCheckboxKeyboardActivation(input)') && str_contains($script, "input.getAttribute('data-ak-company-keyboard-bound') === '1'") && str_contains($script, "event.code !== 'Space'") && str_contains($script, 'input.click();'), 'The remounted registered company checkbox has one scoped Space activation binding that replays exactly one native click without cloning the Woo-owned input.');
...
$test->assert(is_string($functions) && str_contains($functions, 'appleklinika_customer_address_book_checkout_selection') && str_contains($functions, 'appleklinika_checkout_uses_saved_address') && str_contains($functions, 'appleklinika_checkout_company_enabled_for_validation'), 'Server-side checkout validation receives the canonical saved/one-off address mode and company identity through the address-book integration boundary.');
