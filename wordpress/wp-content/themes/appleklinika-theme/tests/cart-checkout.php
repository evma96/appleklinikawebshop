...
$test->assert(is_string($script) && str_contains($script, "['Számlázási cím', addressSummary(billing)]") && str_contains($script, "prefix === 'billing' && sameAddress && sameAddress.checked ? 'shipping' : prefix"), 'Checkout summary uses shipping for billing only when the native same-address control is checked; an independent empty billing address stays empty.');
$test->assert(is_string($script) && str_contains($script, "var taxNumber = address.tax_number ? 'Adószám: ' + address.tax_number : '';") && str_contains($script, 'return [recipient, taxNumber, address.address_1, address.address_2, locality].filter(Boolean).join(\', \');'), 'Checkout Step 3 renders a company tax number only when the effective billing projection contains one.');
$test->assert(is_string($script) && str_contains($script, "var companyTaxNumber = document.getElementById('order-appleklinika-tax_number');") && str_contains($script, "taxNumber = companyTaxNumber ? companyTaxNumber.value.trim() : '';") && str_contains($script, 'tax_number: taxNumber'), 'Checkout Step 3 projects the current validated company tax number from the same billing controls as Step 4.');
$test->assert(is_string($script) && str_contains($script, "var currentBilling = checkoutFormAddress('billing');") && str_contains($script, "if (currentBilling.company) {\n        billing = currentBilling;") && str_contains($script, "} else if (!billing || !billing.address_1) {\n        billing = currentBilling;"), 'Checkout Step 3 uses the latest one-off company billing projection while preserving the effective shared-address fallback.');
$test->assert(is_string($script) && str_contains($script, "var taxNumber = '';\n\n      if (prefix === 'billing')") && str_contains($script, 'if (companyToggle && companyToggle.checked && companyName) {'), 'Personal billing keeps the Step 3 company tax projection empty unless company billing is actively selected.');
...
$test->assert(is_string($script) && str_contains($script, 'clearInactiveBillingValidationErrors') && str_contains($script, 'clearValidationErrors') && str_contains($script, "wc/store/store-notices") && str_contains($script, 'appleklinika:checkout-company-mode-changed'), 'Checkout clears inactive personal or company validation errors through the supported Blocks validation and registered notice stores.');
$test->assert(is_string($script) && str_contains($script, 'billingPersonalNameValidation') && str_contains($script, 'companyNameBillingValidation') && str_contains($script, 'companyTaxNumberValidation'), 'Checkout keeps personal-name and individual company/tax validation cleanup scoped to the current billing identity.');
$test->assert(is_string($script) && str_contains($script, 'clearResolvedCompanyValidationErrors') && str_contains($script, 'companyName.validity.valid') && str_contains($script, 'taxNumber.validity.valid') && str_contains($script, "checkoutNoticesApi") && str_contains($script, "removeNotice(entry.notice.id, entry.context)") && str_contains($script, 'hasCompanyNameError') && str_contains($script, 'hasTaxNumberError'), 'A corrected valid company identity clears only its obsolete Woo Blocks validation message and matching checkout notice.');
$test->assert(is_string($script) && str_contains($script, 'function firstNativeValidationError(step)') && str_contains($script, 'nativeError.reportValidity()') && ! str_contains($script, 'validation.dispatch.showAllValidationErrors();\n      window.setTimeout'), 'The stepper blocks invalid native company or personal fields before asynchronous Woo validation can leave a stale error on a later step.');
...
$test->assert(is_string($css) && str_contains($css, 'ak-checkout-step-3 #shipping-option') && str_contains($css, 'ak-checkout-step-3 #payment-method') && str_contains($css, 'wc-block-components-radio-control__option:has(input:checked)'), 'Step 3 gives the current native checked shipping and payment radios one shared selected-card treatment.');
...
$test->assert(is_string($css) && str_contains($css, '#contact-fields .ak-checkout-profile-save label') && str_contains($css, 'grid-template-columns: 20px minmax(0, 1fr);') && str_contains($css, '.ak-checkout-decision label') && str_contains($css, 'grid-template-columns: minmax(0, 1fr) 42px;') && str_contains($css, '.wc-block-components-sidebar-layout > .ak-checkout-summary-slot') && str_contains($css, 'order: 1;'), 'Checkout separates compact optional preferences from native billing switches, while the mobile summary follows the active form.');
...
$test->assert(is_string($css) && str_contains($css, '.wc-block-checkout__form') && str_contains($css, 'ak-checkout-step-2 .wc-block-checkout__form') && str_contains($css, '#shipping-fields #shipping'), 'Checkout uses one active form surface with scoped internal sections and a stable responsive shipping-address grid.');
