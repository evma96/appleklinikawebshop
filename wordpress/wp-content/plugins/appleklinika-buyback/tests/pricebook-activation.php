...
try {
    ...
    $page = new PriceBooksPage(
        $books, $rules, $catalog, $create, new AppleKlinika\Buyback\Application\Handler\ClonePriceBookToDraftHandler($books, $rules, $transactions, $clock), new AppleKlinika\Buyback\Application\Handler\DiscardDraftPriceBookHandler($books, new AppleKlinika\Buyback\Infrastructure\Persistence\WordPress\WordPressDraftPriceBookDiscardRepository($wpdb), $transactions), new AppleKlinika\Buyback\Application\Handler\SaveDraftBasePriceMatrixHandler($books, $rules, $catalog, $transactions, $clock), new AppleKlinika\Buyback\Application\Handler\SaveDraftModelMinimumOfferHandler($books, $rules, $catalog, $transactions, $clock), new AppleKlinika\Buyback\Application\Handler\SaveDraftQuestionnaireConditionsHandler($books, $rules, $transactions, $clock, new AppleKlinika\Buyback\Application\LocalDemo\LocalDemoQuestionnaire(), $catalog), new AppleKlinika\Buyback\Application\Handler\SaveDraftBatteryBandsHandler($books, $rules, $transactions, $clock, new AppleKlinika\Buyback\Application\LocalDemo\LocalDemoQuestionnaire(), $catalog), new AppleKlinika\Buyback\Application\Handler\SaveDraftOfferModeModifiersHandler($books, $rules, $transactions, $clock), new OfferModeExampleCalculator(new PricingEngine()), new UpdateDraftPriceBookSettingsHandler($books, $clock), $add,
