...
try {
    ...
    $test->assert(str_contains((string) $checkoutScript, 'var initialOption = matchingOption();') && str_contains($checkoutScript, 'if (initialOption) {') && str_contains($checkoutScript, 'setCustomFields(section, initialOption);'), 'checkout applies custom address details to the initially selected default');
    ...
    $test->assert(str_contains((string) $checkoutScript, 'function clearSavedBillingIdentityProjection(section)') && str_contains($checkoutScript, "['billing-company', 'order-appleklinika-company_name', 'order-appleklinika-tax_number']") && str_contains($checkoutScript, "current && current.mode === 'one_off'"), 'a restored one-off billing address retains its physical fields but clears a prior saved company identity.');
