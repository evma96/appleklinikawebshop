...
- Customer-facing WooCommerce system copy is localized through scoped PHP and `wp.i18n` filters, while provider and product names remain dynamic. Checkout exposes one `Pénztár` H1, a semantic four-step progress indicator with a compact mobile presentation, and an effective Step 3 billing summary that remains consistent with Step 4 for personal, company, same-address, and separate-address states.
