...
Back Office regression scripts are command-line only; direct web requests return 404 without running test fixtures. Before any later staging deployment, explicitly configure WordPress's environment type as `staging` and keep external integrations disabled until verified sandbox/test configuration is available. This candidate does not configure or deploy staging.
