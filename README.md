...
The daily-processing refinement adds an actionable work queue, linked live summary counts, explicit payment/attention and next-step cues, a compact order detail with delivery-specific progress, and safe multiline address rendering. Open work excludes physically handed-over/picked-up orders; `Átadva / átvéve` and `Lezárt rendelések` deliberately distinguish physical handover from WooCommerce completion. Order-time device identifiers take priority over current product data, manual notes join employee activity, and COD is labeled as cash on delivery rather than proof of collection. Existing provider-owned invoice/GLS PDFs can be opened through capability- and nonce-protected Back Office actions only when the active integration and real local file are available. No invoice-generation button or duplicate provider business logic is introduced. See the [local UX verification and integration audit](docs/backoffice-daily-processing-qa.md).

Back Office regression scripts are command-line only; direct web requests return 404 without running test fixtures. Staging must explicitly report environment type `staging`; active provider integrations do not authorize generating invoices or labels during smoke testing. A dedicated Back Office host uses the environment-specific `appleklinika_backoffice_origin` WordPress option to keep native login, its host-only cookies, and navigation on that exact host without changing storefront `home`, `siteurl`, permalinks or cookie scope. See the [dedicated-host staging and rollback guide](docs/backoffice-staging-host.md).
