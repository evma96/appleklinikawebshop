...
- `functions.php`: renders the homepage hero, centralized trust/info tile copy, real WooCommerce sale/featured/latest product showcase sections, and real category shortcut links; homepage product sections reuse the shared `appleklinika_render_product_card()` renderer used by the shop archive.
