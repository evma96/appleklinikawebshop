...
- `functions.php` and `assets/js/commerce-localization.js`: localize WooCommerce-owned transaction labels at their PHP and `wp.i18n` extension points. Exact source strings and the `woocommerce` domain are required, so product names and future shipping/payment provider titles are not rewritten.
...
- `assets/js/frontend.js`: resolves the effective billing presentation from the current same-address contract without changing the Blocks order payload, and provides one checkout H1 plus programmatic completed/current/future state for all four progress steps.
