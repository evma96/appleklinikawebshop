...
- Fixed Buyback price-book duplication for long names: the existing 120-byte domain limit remains enforced, while draft cloning now preserves its generated copy/version suffix and safely shortens only the source-name portion when needed.
...
- Added optional draft-only model/offer-mode adjustment overrides. An explicit model rule replaces the corresponding price-book-wide offer-mode adjustment without stacking; absent overrides continue to inherit the global default. The existing pricing-rule table is reused, no schema or price-book-data migration was added, and active/retired books remain immutable.
