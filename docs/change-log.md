...
- Added the private Apple Klinika Back Office MVP plugin with WooCommerce order queues/search, secured order detail pages, internal fulfilment workflow and history, internal notes, a printer-friendly packing sheet, and guarded reuse of the existing GLS label integration without changing stock or storefront behaviour.
- Reworked Back Office queue/search handling to use bounded 25-row WooCommerce pagination, query-level fulfilment filters, HPOS field queries, and new order-time device-identifier snapshots instead of loading and filtering a fixed in-memory order history.
- Added a safe customer-facing order-progress section based on the existing Back Office state and a Back Office-only current-day employee activity view with per-employee action counts.
- Simplified the Back Office into the operational `Új → Előkészítés alatt → Csomagolás alatt → Szállításra előkészítve → Átadva a GLS-nek` flow, with safe compatibility reads for existing legacy states and no manual completion step.
- Restricted the Back Office worklist to submitted WooCommerce operational statuses, excluded checkout drafts and terminal orders, added explicit payment/draft blocks, a guided primary action, visible PRG feedback, a status select, summary counts, and a minimal employee header.
- Fixed UX-BO-001: Back Office order links, the visible return action, and state/note PRG redirects now preserve only validated queue, search, search-type, and page context, without accepting arbitrary return URLs.
