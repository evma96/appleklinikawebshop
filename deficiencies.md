...
- Product gallery multi-image behavior now has a LOCAL browser regression using the real PHP renderer with an unsaved product and existing media (no database product/gallery edits). Published local products currently have no multi-image galleries. A real multi-photo catalogue acceptance on the target deployment and physical mobile Safari remain unverified; Chromium at 1440px/390px, native touch taps, pointer swipe/pan and viewport changes pass. See `docs/qa/product-gallery-ux.md`.
