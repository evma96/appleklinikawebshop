...
- The Back Office can invoke the existing GLS plugin only when its account credentials and sender configuration are valid. This worktree did not start a separate Docker runtime, so real label creation and secure-label download still require a controlled local runtime check.
...
- Back Office GLS label creation still depends on the existing shipping plugin's local credentials and sender configuration. This pass deliberately does not test or add live GLS API behavior; a label remains distinct from physical courier handover.
- The Back Office "Today's activity" view deliberately retains only the current day's operational events in a short-lived WordPress transient; it is not a historical reporting record and needs a local runtime check with multiple employees.
