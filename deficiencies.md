...
- Phase 3 must define immutable request/offer linkage to the resolved active price-book identity and version before any public calculator or customer offer is introduced; the Phase 2B1 admin preview remains transient, and legacy `appleklinika_buyback_records` data remains read-only until a separately approved import design exists.
...
- Back Office runtime QA now uses the isolated `appleklinika-backoffice` Docker project at localhost:18080. The installed invoice and GLS integrations are inactive; positive provider PDF checks use isolated adapter fixtures, while real generated-document opening/printing and generation still require verified test integration configuration.
- Back Office IMEI/internal-device lookup is exact and automatically snapshotted only for orders created after the Back Office plugin is active. A historical backfill would need a separately approved, bounded migration rather than an unbounded order-history scan.
- Back Office GLS label creation requires valid account/sender configuration and sandbox mode on local/test hosts (including localhost with WordPress's default `production` environment label). No real provider call was made. Invoice generation remains unavailable until a verified invoice test account and safe generation semantics are established. Legacy GLS direct-URL metadata is reported unavailable, not fetched or migrated.
- Before any later staging deployment, WordPress must explicitly report environment type `staging`; a non-localhost staging domain left at WordPress's default `production` value cannot be identified as staging by the GLS safety guard. Do not copy production provider credentials or activate integrations as part of deploying this candidate.
- Dedicated Back Office hosting requires an explicitly trusted `appleklinika_backoffice_origin` option and the existing proxy route. Sessions remain host-only, so storefront login is not shared automatically. The verified fix changes generated URLs only for exact dedicated-host requests, not global WordPress addresses, permalinks, cookie domains or permissions. Future proxy, WordPress or login-plugin changes should repeat the native login/reload/logout and denied-user checks in `docs/backoffice-staging-host.md`.
- The Back Office "Today's activity" view deliberately retains only the current day's operational events in a short-lived WordPress transient; it is not a historical reporting record. Single-employee workflow/note activity is runtime verified; multi-employee grouping is covered by isolated regression fixtures, not simultaneous employee sessions. Concurrent day-index updates still use the existing transient mechanism.
- Back Office return context intentionally covers only the current queue, search text, search type, and page. Any future list sort, view, or bulk-selection state needs its own separately scoped preservation rule.
- Automatic Back Office search treats an all-numeric value of up to six digits as an order number; to retain the existing seven-or-more-digit phone recognition, larger numeric order IDs need a leading `#` or the explicit `Rendelésszám` search type.
- Back Office intentionally blocks an order whose canonical WooCommerce shipping method ID is not an approved GLS ID or `local_pickup`; any additional delivery method needs an explicit mapping before it can enter fulfilment.
- The isolated runtime's existing Apache rules route `/backoffice/` but not `/fiokom/`; the normal My Account URL returns an Apache 404. Customer progress privacy is verified through the actual WooCommerce renderer with real local order data and isolated view tests, but full My Account browser navigation requires a separately scoped local rewrite correction.
- WooCommerce's `is_paid()` is status-based, including processing COD orders. The Back Office now labels COD explicitly and does not claim collection, but does not introduce a separate cash-reconciliation operation. Physical handover and WooCommerce completion remain independent facts with separate filters.
